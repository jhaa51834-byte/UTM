...
def test_generate_endpoint(client):
    resp = client.post("/api/v1/generate-utm", json={
...
def test_history_records_and_filters(client):
    client.post("/api/v1/generate-utm", json={
    ...
    assert len(client.get("/api/v1/history").json()) == 2
    assert len(client.get("/api/v1/history", params={"source": "google"}).json()) == 1
    assert client.get("/api/v1/history", params={"q": "beta"}).json()[0]["utm_campaign"] == "beta"


def test_template_roundtrip(client):
    resp = client.post("/api/v1/save-template", json={
...
def test_governance_rules_require_admin(client):
    ...
    assert client.post("/api/v1/governance-rules", json=payload).status_code == 403
    assert client.post("/api/v1/governance-rules", json=payload,
...
def test_bulk_generate(client):
    ...
    resp = client.post("/api/v1/bulk-generate",
...
def test_qr_endpoint(client):
    resp = client.get("/api/v1/qr", params={"url": "https://example.com/p?utm_source=x"})
    assert resp.status_code == 200
    assert resp.headers["content-type"] == "image/png"
    svg = client.get("/api/v1/qr", params={"url": "https://example.com/p?utm_source=x",
                                        "fmt": "svg"})
